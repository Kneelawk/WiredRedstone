...
import com.kneelawk.wiredredstone.part.AbstractTwoInputGatePart
import com.kneelawk.wiredredstone.part.GateXnorPart
import com.kneelawk.wiredredstone.part.GateXorPart
...
sealed class GateXorBlockNode : AbstractGateBlockNode<GateXorPart>(GateXorPart::class) {
    ...
    override fun getType() = WRBlockNodes.GATE_XNOR

    override fun toTag(): NbtElement? = BlockNodeUtil.writeSidedType(side, type, ::writeExtra)

    data class Input(private val side: Direction, private val inputType: AbstractTwoInputGatePart.InputType) :
        GateXorBlockNode() {
        ...
        override fun getConnectDirection(part: GateXorPart) = part.getInputSide(inputType)
    ...
    data class Output(private val side: Direction) : GateXorBlockNode() {
        override val type = Type.OUTPUT

        override fun getSide() = side

        override fun getConnectDirection(part: GateXorPart) = part.getOutputSide()
    ...
    object Decoder : BlockNodeDecoder {
        override fun decode(tag: NbtElement?): BlockNode? {
            return BlockNodeUtil.readSidedTyped<Type, _>(tag) { side, type, tag ->
                when (type) {
                    Type.INPUT -> Input(side, tag.getByte("inputType").toEnum())
