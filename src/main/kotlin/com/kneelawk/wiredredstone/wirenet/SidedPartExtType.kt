...
interface SidedPartExtType : PartExtType {
    override fun createExtsForContainer(world: World, pos: BlockPos, part: NetNodeContainer): Set<PartExt> {
        return (part as? SidedPart)?.let { createExtsForPart(world, SidedPos(pos.toImmutable(), it.side), it) }
            ?: emptySet()
    }

    fun createExtsForPart(world: World, pos: SidedPos, part: SidedPart): Set<PartExt>
}