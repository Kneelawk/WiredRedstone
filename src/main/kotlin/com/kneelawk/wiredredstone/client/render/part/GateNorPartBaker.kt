...
object GateNorPartBaker : AbstractPartBaker<GateNorPartKey>() {
    ...
    override fun renderOverlayText(
        key: GateNorPartKey, stack: MatrixStack, provider: VertexConsumerProvider
    ) {
        RenderUtils.renderPortText(
            overlay("gate_nor.out"), key.side, key.direction, 2.0 / 16.0, stack, provider
        )
        RenderUtils.renderPortText(
            overlay("gate_nor.in"), key.side, cardinalRotatedDirection(EAST, key.direction), 2.0 / 16.0,
            stack, provider, portText(key.inputRightEnabled)
        )
        RenderUtils.renderPortText(
            overlay("gate_nor.in"), key.side, cardinalRotatedDirection(SOUTH, key.direction), 2.0 / 16.0,
            stack, provider, portText(key.inputBackEnabled)
        )
        RenderUtils.renderPortText(
            overlay("gate_nor.in"), key.side, cardinalRotatedDirection(WEST, key.direction), 2.0 / 16.0,
