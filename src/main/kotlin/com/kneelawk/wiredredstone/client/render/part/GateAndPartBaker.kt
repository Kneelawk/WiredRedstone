...
object GateAndPartBaker : AbstractPartBaker<GateAndPartKey>() {
    private val BACKGROUND = WRConstants.id("block/gate_and/background")
    private val INPUT_RIGHT_ON = WRConstants.id("block/gate_and/redstone_input_right_on")
    private val INPUT_RIGHT_OFF = WRConstants.id("block/gate_and/redstone_input_right_off")
    private val INPUT_BACK_ON = WRConstants.id("block/gate_and/redstone_input_back_on")
    private val INPUT_BACK_OFF = WRConstants.id("block/gate_and/redstone_input_back_off")
    private val INPUT_LEFT_ON = WRConstants.id("block/gate_and/redstone_input_left_on")
    private val INPUT_LEFT_OFF = WRConstants.id("block/gate_and/redstone_input_left_off")
    private val TORCH_RIGHT_ON = WRConstants.id("block/gate_and/torch_input_right_on")
    private val TORCH_RIGHT_OFF = WRConstants.id("block/gate_and/torch_input_right_off")
    private val TORCH_BACK_ON = WRConstants.id("block/gate_and/torch_input_back_on")
    private val TORCH_BACK_OFF = WRConstants.id("block/gate_and/torch_input_back_off")
    private val TORCH_LEFT_ON = WRConstants.id("block/gate_and/torch_input_left_on")
    private val TORCH_LEFT_OFF = WRConstants.id("block/gate_and/torch_input_left_off")
    private val ANODE_ON = WRConstants.id("block/gate_and/redstone_anode_on")
    private val ANODE_OFF = WRConstants.id("block/gate_and/redstone_anode_off")
    private val TORCH_OUTPUT_ON = WRConstants.id("block/gate_and/torch_output_on")
    private val TORCH_OUTPUT_OFF = WRConstants.id("block/gate_and/torch_output_off")
    ...
    override fun renderOverlayText(
        ...
        RenderUtils.renderPortText(
            overlay("gate_and.in"), key.side, cardinalRotatedDirection(EAST, key.direction), 2.0 / 16.0, stack,
            provider, light
        )
        RenderUtils.renderPortText(
            overlay("gate_and.in"), key.side, cardinalRotatedDirection(SOUTH, key.direction), 2.0 / 16.0, stack,
            provider, light
        )
        RenderUtils.renderPortText(
            overlay("gate_and.in"), key.side, cardinalRotatedDirection(WEST, key.direction), 2.0 / 16.0, stack,
            provider, light
        )
    }
}
