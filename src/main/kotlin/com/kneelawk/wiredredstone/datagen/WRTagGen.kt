...
class WRTagGen(output: FabricDataOutput, registries: CompletableFuture<HolderLookup.Provider>) :
    ...
    companion object {
        ...
        val COLORED_BUNDLED_CABLES = TagKey.of(RegistryKeys.ITEM, id("colored_bundled_cables"))
        val INSULATED_WIRES = TagKey.of(RegistryKeys.ITEM, id("insulated_wires"))
    }
    ...
    override fun configure(arg: HolderLookup.Provider) {
        ...
        getOrCreateTagBuilder(COLORED_BUNDLED_CABLES).let {
            for (color in DyeColor.values()) {
                it.add(WRItems.BUNDLED_CABLES[color]!!)
            }
        }
        getOrCreateTagBuilder(INSULATED_WIRES).let {
            for (color in DyeColor.values()) {
                it.add(WRItems.INSULATED_WIRES[color]!!)
            }
        }
    }
}
